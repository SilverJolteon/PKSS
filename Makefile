...
APP_TITLE	:=	$(TARGET)
APP_DESCRIPTION	:=	Pokemon Save Selector
...
CXXFLAGS	:= $(CFLAGS) -fno-rtti -fno-exceptions -std=gnu++11 -Wno-maybe-uninitialized
...
-icon $(TOPDIR)/icon.bin -banner $(TOPDIR)/banner.bin -romfs "$(BUILD)/romfs.bin" -exefslogo -target t
...
#---------------------------------------------------------------------------------
clean:
	@rm -fr $(BUILD) $(TARGET).3dsx $(OUTPUT).smdh $(OUTPUT).elf $(OUTPUT).cia $(ROMFS) $(GFXBUILD)
	@clear
...
ifneq ("$(wildcard $(TARGET).elf)","")
	...
	@bannertool makebanner -i $(TOPDIR)/$(BANNER) -a $(TOPDIR)/$(AUDIO) -o $(TOPDIR)/banner.bin
	@3dstool -ctf romfs "$(BUILD)/romfs.bin" --romfs-dir "$(ROMFS)"
	@makerom -f cia -o $(OUTPUT).cia $(BUILD_ARGS)
	@rm -fr icon.bin banner.bin
	@echo built ... $(TARGET).cia
	@echo Cleaning temp files...
	@echo Done!	
else
	@echo $(TARGET).elf not built yet!
endif
#---------------------------------------------------------------------------------
