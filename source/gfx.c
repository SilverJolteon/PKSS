...
int DisplaySaves(){
	int y, pagenum = ceil(entries[num] / (float)5);
	while(aptMainLoop()){
		hidScanInput();
		...
		for(((y = ((6 * page) - 6))); y < ((((entries[num] - 6 * page) >= 0) * 6 * page) + (((entries[num] - 6 * page) < 0) * ((6 * (page - 1)) + entries[num] - 5 * (page - 1)))); y++){
		...
		for(y = 0; y <= entries[num]; y++){
			sftd_draw_textf(font, 45, (y + 1) * fontspace + 6, RGBA8(32, 32, 32, 255), 14, "%d: %s", y + 1, saves[num][y]);
		}
		*/
		sf2d_end_frame();
		sf2d_swapbuffers();
